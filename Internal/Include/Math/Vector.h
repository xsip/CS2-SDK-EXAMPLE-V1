...
struct  Vector2D_t
{
	 Vector2D_t(const float x = 0.0f, const float y = 0.0f);
...
struct  Vector
{
	 ...
	 Vector(const Vector2D_t& vecBase2D);
...
#pragma region vector_assignment_operators
	 ...
	 Vector& operator=(const Vector2D_t& vecBase2D);
...
#pragma region vector_arithmetic_assignment_operators
	 ...
	 Vector& operator+=(const float flAdd);

	 Vector& operator-=(const float flSubtract);

	 Vector& operator*=(const float flMultiply);

	 Vector& operator/=(const float flDivide);
...
#pragma endregion
	...
	[[nodiscard]]  float DotProduct(const Vector& vecDot) const;
...
struct  Vector4D_t
{
	 Vector4D_t(const float x = 0.0f, const float y = 0.0f, const float z = 0.0f, const float w = 0.0f) :
		x(x), y(y), z(z), w(w) {
	}
	Vector ToVector();
...
struct alignas(16)  VectorAligned_t : Vector
{
	...
	explicit VectorAligned_t(const Vector& vecBase);
	 ...
	 VectorAligned_t& operator=(const Vector& vecBase);
