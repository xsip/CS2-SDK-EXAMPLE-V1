...
QAngle_t::QAngle_t(float x, float y, float z ) :
	x(x), y(y), z(z) {
}

QAngle_t::QAngle_t(const float* arrAngles) :
...
#pragma region qangle_assignment_operators
 ...
 QAngle_t& QAngle_t::operator=(const QAngle_t& angBase)
{
	this->x = angBase.x;
	this->y = angBase.y;
	this->z = angBase.z;
 ...
 QAngle_t& QAngle_t::operator/=(const QAngle_t& angBase)
{
 ...
 QAngle_t& QAngle_t::operator+=(const float flAdd)
{
	this->x += flAdd;
	this->y += flAdd;
	this->z += flAdd;
 ...
 QAngle_t& QAngle_t::operator-=(const float flSubtract)
{
	this->x -= flSubtract;
	this->y -= flSubtract;
	this->z -= flSubtract;
 ...
 QAngle_t& QAngle_t::operator*=(const float flMultiply)
{
	this->x *= flMultiply;
	this->y *= flMultiply;
	this->z *= flMultiply;
 ...
 QAngle_t& QAngle_t::operator/=(const float flDivide)
{
	this->x /= flDivide;
	this->y /= flDivide;
	this->z /= flDivide;
 ...
 QAngle_t& QAngle_t::operator-()
{
 ...
 QAngle_t QAngle_t::operator-() const
{
...
#pragma region qangle_arithmetic_ternary_operators
 ...
 QAngle_t QAngle_t::operator+(const QAngle_t& angAdd) const
{
	return { this->x + angAdd.x, this->y + angAdd.y, this->z + angAdd.z };
}

 QAngle_t QAngle_t::operator-(const QAngle_t& angSubtract) const
{
	return { this->x - angSubtract.x, this->y - angSubtract.y, this->z - angSubtract.z };
}

 QAngle_t QAngle_t::operator*(const QAngle_t& angMultiply) const
{
	return { this->x * angMultiply.x, this->y * angMultiply.y, this->z * angMultiply.z };
}

 QAngle_t QAngle_t::operator/(const QAngle_t& angDivide) const
{
	return { this->x / angDivide.x, this->y / angDivide.y, this->z / angDivide.z };
}

 QAngle_t QAngle_t::operator+(const float flAdd) const
{
	return { this->x + flAdd, this->y + flAdd, this->z + flAdd };
}

 QAngle_t QAngle_t::operator-(const float flSubtract) const
{
	return { this->x - flSubtract, this->y - flSubtract, this->z - flSubtract };
}

 QAngle_t QAngle_t::operator*(const float flMultiply) const
{
	return { this->x * flMultiply, this->y * flMultiply, this->z * flMultiply };
}

 QAngle_t QAngle_t::operator/(const float flDivide) const
{
...
Vector QAngle_t::AngleToDirection() {
	 float DEG2RAD_F = 0.017453292519943295769f; // π/180
